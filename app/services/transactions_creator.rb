class TransactionsCreator
  def self.create_transactions(user, document, raw_transactions)
    raw_transactions.each do |transaction|
      ...
      user.transactions.create!(document: document, merchant: merchant, amount: transaction.amount, 
        charge_amount: transaction.charge_amount, txn_date: transaction.date)
    end
  end
end